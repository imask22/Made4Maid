...
<!--    <h1 class="pt-5">Welcome     <%=(String)session.getAttribute("USERID")%> !!! </h1>-->
    <h1 class="pt-5">"REQUESTS DENIED BY CUSTOMERS"</h1>
    <hr class="w-75 mx-auto pt-3">
    
			
    <div class="container text-left">
<%
whydeny wd = new whydeny();
whydenyDao wdo = new whydenyDao();
ArrayList<whydeny> lst = new ArrayList<>();
lst = wdo.fetchDenyReasonmaid();
if(lst!=null)
{
	for(whydeny w : lst)
	{
		%>
                <h4 clss="text-bold">POST ID: <%=w.getPid() %></h4>
                
                <h4 clss="text-bold">POST_DESC: <%
                    String temp= w.getWhyDesc();
                    if(temp==null){
                ...
                <h4 clss="text-bold">CUSTOMER_ID:<%=w.getMaid() %></h4>
		<hr>
		<%
	}
}
