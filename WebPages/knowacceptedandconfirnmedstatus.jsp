...
<div class="container my-5 text-center text-capatalize">
    ...
    <h1 class="pt-5">"ACCEPTED & CONFIRMED STATUS "</h1>
...
lst=pd.fetchMyConnection();
if(lst!=null)
{
	for(connection c1: lst)
	{
		%>
                <h4 clss="text-bold">POST_ID: <%=c1.getPostID() %></h4>
                <h4 clss="text-bold">DATE & TIME: <%=c1.getCustID() %></h4>
                <h4 clss="text-bold">MAID_ID: <%=c1.getMaidID() %></h4>
                <h4 clss="text-bold">CUSTOMER_ID:<%=c1.getPostedDate() %></h4>
		<hr>
		<%
	}
}
