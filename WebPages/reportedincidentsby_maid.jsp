...
<div class="container my-5 text-center text-capatalize">
    ...
    <h1 class="pt-5">"INCIDENTS REPORTED BY MAIDS"</h1>
...
<%
reportincidentsbymaidDao cd = new reportincidentsbymaidDao();
ArrayList<reportedincidentsbymaid> lst = new ArrayList<>();
lst=cd.getAllReportedIncidentsByMaid();
if(lst!=null)
{
	for(reportedincidentsbymaid obj : lst)
	{
		...
		%>
                ...
                <h4 clss="text-bold">INCIDENT DESCRIPTION:- <%=obj.getIncDesc() %></h4>
                <h4 clss="text-bold">MAID ID:- <%=obj.getMaidID()%></h4>
                <h4 clss="text-bold">CUST ID:- <%=obj.getCustomerID() %></h4>
                    </div>
                      
                    <div class="col-6 col-lg-6 col-sm-6 text-right">
                <h5 class="text-bold"><a href = "blockthiscustomer.jsp?custID=<%=obj.getCustomerID()%>">BLOCK THIS CUSTOMER!!</a></h5>
                    </div>
